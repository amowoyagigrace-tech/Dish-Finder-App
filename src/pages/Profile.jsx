...
export default function Profile() {
  ...
  const savePreferences = async () => {
    ...
    try {
      if (profile) {
        await UserProfile.update(profile.id, prefs);
      } else {
        const created = await UserProfile.create({ ...prefs, subscription_tier: "free", searches_used: 0 });
        setProfile(created);
      }
      setSaved(true);
    ...
    <div className="px-5 py-6 max-w-lg mx-auto pb-10">
      ...
      <div className="bg-card border border-border rounded-2xl p-4 mb-6 space-y-5">
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {DIETARY_OPTIONS.map(opt => (
              <button key={opt} onClick={() => setPrefs(prev => ({ ...prev, dietary_preference: opt }))}
                className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${prefs.dietary_preference === opt ? "bg-primary text-primary-foreground border-primary" : "bg-secondary border-border text-muted-foreground hover:text-foreground"}`}>
          ...
          <div className="flex flex-wrap gap-2">
            {CUISINES.map(c => (
              <button key={c} onClick={() => toggleCuisine(c)}
                className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${prefs.preferred_cuisines.includes(c) ? "bg-primary text-primary-foreground border-primary" : "bg-secondary border-border text-muted-foreground hover:text-foreground"}`}>
                {c}
              </button>
            ))}
          </div>
        ...
        <div>
          ...
          <input type="range" min={1} max={25} step={0.5} value={prefs.default_radius}
      ...
      <div className="rounded-2xl border border-border overflow-hidden mb-6">
        <Link to="/terms" className="flex items-center gap-3 bg-card px-4 py-4 border-b border-border hover:bg-muted transition-colors">
          <FileText className="w-4 h-4 text-muted-foreground" />
          <span className="flex-1 text-sm text-foreground">Terms of Service</span>
          <ChevronRight className="w-4 h-4 text-muted-foreground" />
        </Link>
        <Link to="/privacy" className="flex items-center gap-3 bg-card px-4 py-4 hover:bg-muted transition-colors">
